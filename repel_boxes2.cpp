...
DataFrame repel_boxes2(
    ...
    NumericVector xlim, NumericVector ylim,
    double force = 1e-6, int maxiter = 128000,
    std::string direction = "both"
) {
    ...
    for (int i = 0; i < n_points; i++) {
      ...
      for (int j = i + 1; j < n_points; j++) {
        ...
        if (overlaps(NodeBoxes[i], NodeBoxes[j])) {
          ...
          std::cout << i << "and" << j << "intersect! (N)" << std::endl;
          }
      ...
      if (!any_overlaps) {
        f = f + spring_force(noriginal_centroids[i], ci, force * 1, direction);
        }
    ...
    for (int i = 0; i < e_points; i++) {
      ...
      for (int j = i + 1; j < e_points; j++) {
        ...
        if (overlaps(EdgeBoxes[i], EdgeBoxes[j])) {
          ...
          std::cout << i << "and" << j << "intersect! (E)" << std::endl;
          }
        }


      if (!any_overlaps) {
        ef = ef + spring_force(eoriginal_centroids[i], eci, force * 1, direction);
        }
    ...
    for (int i = 0; i < n_points; i++) {
      ...
      for (int j = 0; j < e_points; j++) {
        ...
        if (overlaps(NodeBoxes[i], EdgeBoxes[j])) {
          ...
          std::cout << i << "and" << j << "intersect! (N-E)" << std::endl;
          }
        }

      if (!any_overlaps) {
        bf = bf + spring_force(noriginal_centroids[i], bci, force * 1, direction);
        }
